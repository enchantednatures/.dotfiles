...
function Slayout:configure_window_close_on_switch(winid, is_editor)
  ...
  utils.create_singleton_autocmd({ "BufWinEnter", "BufReadPost", "BufNewFile" }, {
    window = winid,
    callback = function(event) action(winid, event.buf, event.file) end,
  })
end
...
function Slayout:open_popup()
  ...
  local configure_mappings = function(winid, other_winid)
    ...
    vim.keymap.set("n", "<ESC>", function() close_popup() end, { silent = true, buffer = bufnr })

    -- set keymaps for switching popups from one to the other
    vim.keymap.set(
      "n",
      "l",
      function() vim.api.nvim_set_current_win(other_winid) end,
...
---@return boolean
function Slayout:is_open() return self.is_opened end
...
function Slayout:open()
  ...
  -- todo: have a way to open the results in a new tab from the query and vice versa
  -- vim.keymap.set("n", "tn", ":tabe " .. editor_bufnr .. "<cr>", { silent = true, buffer = true })
  vim.keymap.set("n", "tn", ":tabe %<cr>", { silent = true, buffer = true })
  vim.keymap.set("n", "qq", function() self:close() end, { silent = true, buffer = true })
  ...
  -- vim.keymap.set("n", "tn", ":tab sb " .. editor_bufnr .. "<cr>", { silent = true, buffer = true })
  -- vim.keymap.set("n", "tr", ":tab sb " .. result_bufnr .. "<cr>", { silent = true, buffer = true })
